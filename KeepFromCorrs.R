KeepFromCorrs <- function(input,threshold,optimize.me){
  ...
  if(class(input)=="data.frame"){
    ...
    for(i in 1:nrow(p.pairs)){
      p.pairs$longcomp[i] <- length(p.pairs[i,1:2][!is.na(p.pairs[i,1:2])])
    }
    ...
    while(any(counts$Freq>1)){
      ...
      for(i in 1:nrow(p.pairs)){
        p.pairs$longcomp[i] <- length(p.pairs[i,1:2][!is.na(p.pairs[i,1:2])])
        p.pairs$longcomp[i] <- as.numeric(p.pairs$longcomp[i])
      }
      char.2membs <- as.character(unlist(p.pairs[which(p.pairs$longcomp>1),1:2]))
      counts <- as.data.frame(table(char.2membs))
    }
    ...
    if(optimize.me==TRUE){
      for(i in 1:nrow(p.pairs)){
        if(length(p.pairs[i,1:2][!is.na(p.pairs[i,1:2])])>1){
          p.pairs[i,which(p.pairs[i,4:5]==min(p.pairs[i,4:5]))] <- NA
        }
      }
